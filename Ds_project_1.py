...
def add_bg_from_url():
    st.markdown(
         ...
         <style>
         .stApp {{
             background-image: url("https:https://wallpapercave.com/download/forex-wallpapers-wp2116598");
...
COMPANY = st.selectbox("Select Company from Below list",(' ','HDFC','BANKNIFTY','3MINDIA','AARTIDRUGS','AARTIIND','AAVAS','ABB','ABCAPITAL','ABFRL','ACC',
							   ...
							   'GSPL','GUJALKALI','GUJGASLTD','GULFOILLUB','HAL','HATHWAY','HATSUN','HAVELLS','HCLTECH','NIFTY',
...
MODEL = st.selectbox('Select Forecasting Model From Below List',(' ','Model Based','Data Driven','ARIMA','LSTM Artificial Neural Network','FB Prophet'))

##########################################################################################################################################

def baseplots(var):
    tv = TvDatafeed()
    data = tv.get_hist(symbol='ICICIBANK',exchange='NSE',n_bars=5000)
